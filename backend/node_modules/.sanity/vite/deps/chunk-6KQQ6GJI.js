...
function normalizeTimeout(time) {
  ...
  return isNaN(delay2) ? normalizeTimeout(defaultOptions.timeout) : { connect: delay2, socket: delay2 };
}
  ...
  if (resultSelector) {
    return function(source) {
      return source.pipe(exhaustMap(function(a, i) {
        return innerFrom(project(a, i)).pipe(map(function(b2, ii) {
          return resultSelector(a, b2, i, ii);
        }));
      }));
    };
...
var Cancel = class {
  constructor(message) {
    this.__CANCEL__ = true, this.message = message;
  }
  toString() {
...
var defaultShouldRetry = (err, attempt, options) => options.method !== "GET" && options.method !== "HEAD" ? false : err.isNetworkError || false;
...
var sharedRetry = (opts) => {
  ...
  return {
    onError: (err, context2) => {
      const options = context2.options, max2 = options.maxRetries || maxRetries, shouldRetry2 = options.shouldRetry || allowRetry, attemptNumber = options.attemptNumber || 0;
      if (isStream(options.body) || !shouldRetry2(err, attemptNumber, options) || attemptNumber >= max2)
        return err;
...
var retry2 = (opts = {}) => sharedRetry({ shouldRetry: defaultShouldRetry, ...opts });
retry2.shouldRetry = defaultShouldRetry;
function buildKeepAlive(agent2) {
  return function(config2 = {}) {
    const ms = config2.ms || 1e3, maxFree = config2.maxFree || 256;
    return agent2({
      keepAlive: true,
      keepAliveMsecs: ms,
...
// node_modules/@sanity/client/dist/_chunks/browserMiddleware.js
...
function shouldRetry(err, attempt, options) {
  ...
  return (isSafe || isQuery) && isRetriableResponse ? true : retry2.shouldRetry(err, attempt, options);
}
...
function I(t) {
  return Number.isNaN(Number(t)) ? !!Date.parse(t) : false;
}
function x(t) {
  try {
...
function _fetch(client, httpRequest, _stega, query, _params = {}, options = {}) {
  ...
  } : _stega, params = stega.enabled ? vercelStegaCleanAll(_params) : _params, mapResponse = options.filterResponse === false ? (res) => res : (res) => res.result, { cache, next, ...opts } = {
  ...
  return stega.enabled ? $request.pipe(
    combineLatestWith(
      from(
        import("./stegaEncodeSourceMap-SQNPDLX5.js").then(function(n) {
          return n.a;
        }).then(
...
function _requestObservable(client, httpRequest, options) {
  ...
  if (tag && options.tag !== null && (options.query = { tag: requestTag(tag), ...options.query }), ["GET", "HEAD", "POST"].indexOf(options.method || "GET") >= 0 && uri.indexOf("/data/query/") === 0) {
    ...
    typeof perspective == "string" && perspective !== "raw" && (validateApiPerspective(perspective), options.query = { perspective, ...options.query }, perspective === "previewDrafts" && useCdn && (useCdn = false, printCdnPreviewDraftsWarning())), options.returnQuery === false && (options.query = { returnQuery: "false", ...options.query });
  }
...
var AssetsClient = class {
  ...
  upload(assetType, body, options) {
    const observable22 = _upload(__privateGet$4(this, _client2$3), __privateGet$4(this, _httpRequest2$4), assetType, body, options);
...
function _listen(query, params, opts = {}) {
  ...
  return (token || withCredentials) && (esOptions.withCredentials = true), token && (esOptions.headers = {
    ...
    let reconnectTimer, stopped = false;
    function onError() {
    ...
    async function getEventSource() {
      const { default: EventSource } = await import("./browser-N7NE4VPU.js"), evs = new EventSource(uri, esOptions);
...
function defineCreateClientExports(envMiddleware2, ClassConstructor) {
  return { requester: defineHttpRequest(envMiddleware2, {}).defaultRequester, createClient: (config2) => new ClassConstructor(
...
is-plain-object/dist/is-plain-object.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-6KQQ6GJI.js.map
